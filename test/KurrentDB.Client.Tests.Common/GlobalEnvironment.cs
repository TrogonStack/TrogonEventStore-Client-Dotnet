...
public static class GlobalEnvironment {
	public const string DefaultDockerImage = "ghcr.io/trogonstack/trogoneventstore:ci";
	...
	static GlobalEnvironment() {
		...
		static void EnsureDefaults(IConfiguration configuration) {
			// internal defaults
			configuration.EnsureValue("TESTCONTAINER_KURRENTDB_IMAGE", DefaultDockerImage);
